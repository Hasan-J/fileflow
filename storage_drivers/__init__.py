...
from .sftp_storage_driver import SFTPStorageDriver
...
def get_storage_driver(
    ...
    :param str storage_type: The storage type settings. Currently supports
        'file' or 's3'.
    :param str storage_prefix: The file or sftp storage prefix. Becomes the base path
        for file storage or sftp remote dir for sftp storage.
    ...
    if storage_type == 'file':
        ...
        if sftp_host is None:
            sftp_host = configuration.get('fileflow', 'sftp_host')

        if sftp_port is None:
            sftp_port = configuration.getint('fileflow', 'sftp_port')

        if sftp_username is None:
            sftp_username = configuration.get('fileflow', 'sftp_username')

        if sftp_password is None:
            sftp_password = configuration.get('fileflow', 'sftp_password')

        return SFTPStorageDriver(
            host=sftp_host,
            port=sftp_port,
            username=sftp_username,
            password=sftp_password,
            sftp_root_dir=storage_prefix
        )
...
__all__ = ['StorageDriver', 'StorageDriverError', 'FileStorageDriver', 'SFTPStorageDriver', 'S3StorageDriver', 'get_storage_driver']
